...
class AppversionHandler(webapp.RequestHandler):
    ...
    def post(self, func_name):
        ...
        func = getattr(self, '_RpcCall_'+func_name, None)
        if func:
            try:
    ...
    def _RpcCall_create(self, app_id, version, path, data):
    ...
    def _RpcCall_clonefiles(self, app_id, version, path, data):
        files = self._extractFileTuples(data)
        self.response.out.write(
            LIST_DELIMITER.join([filename for filename, _ in files]))

    def _RpcCall_cloneblobs(self, app_id, version, path, data):
        files = self._extractFileTuples(data)
        self.response.out.write(
            LIST_DELIMITER.join([filename for filename, _, mimetype in files]))

    def _RpcCall_addfile(self, app_id, version, path, data):
    ...
    def _RpcCall_addfiles(self, app_id, version, path, data):
        appversion = self.getAppversion(app_id, version, STATE_UPDATING)
        app_dir = self.getAppversionDirectory(appversion)
        self.extractFiles(data, app_dir)

    def _RpcCall_addblobs(self, app_id, version, path, data):
        appversion = self.getAppversion(app_id, version, STATE_UPDATING)
        app_dir = self.getAppversionDirectory(appversion)
        self.extractFiles(data, app_dir)

    def _RpcCall_deploy(self, app_id, version, path, data):
        appversion = self.getAppversion(app_id, version, STATE_UPDATING)
        app_dir = self.getAppversionDirectory(appversion)
    ...
    def _RpcCall_isready(self, app_id, version, path, data):
        self.response.out.write('1')

    def _RpcCall_rollback(self, app_id, version, path, data):
        appversion = self.getAppversion(app_id, version, STATE_UPDATING)
        app_dir = self.getAppversionDirectory(appversion)
