...
class FileBlobStorage(
    ...
    def _BlobKey(cls, blob_key):
        ...
        if not isinstance(blob_key, google.appengine.api.blobstore.BlobKey):
            return google.appengine.api.blobstore.BlobKey(unicode(blob_key))
        return blob_key
    ...
    def StoreBlob(self, blob_key, blob_stream):
        ...
        Args:
            ...
            blob_stream: Stream or stream-like object that will generate
                blob content.
        """
        blob_key = self._BlobKey(blob_key)
